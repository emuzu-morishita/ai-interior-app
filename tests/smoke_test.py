...
# レポート3形式を直接ビルドして検証（placement が含まれること・形式が壊れていないこと）
...
kwargs = dict(lang="ja", room_label=ROOM_SIZE_LABELS["ja"][0], taste="北欧モダン",
...
assert pdf_out[:4] == b"%PDF", "PDFが壊れている"
xlsx_out = build_excel_report(ITEMS, shopping, room_img, **kwargs)
assert xlsx_out[:2] == b"PK", "Excelが壊れている"
print("[4] レポート3形式（placement 入り）OK")
