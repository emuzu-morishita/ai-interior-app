...
1. **選んだ商品で予算を再計算する機能** — 各提案アイテムのおすすめ商品（楽天/Yahoo!）の中からユーザーが「この商品にする」を選ぶと、AIの概算価格ではなく選択した実商品の価格で合計金額・予算差・予算内訳チャートを再計算する。実装時の想定: `st.session_state` に選択状態（item_name → 選択した `ShoppingItem`）を保持し、合計計算を「選択済みは実価格・未選択はAI概算」のハイブリッドにする。レポート出力（HTML/PDF/Excel）にも選択商品を反映させること。
