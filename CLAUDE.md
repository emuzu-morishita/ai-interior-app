...
実装の際は着手前にユーザーへ仕様確認をすること。

1. **選んだ商品で予算を再計算する機能** ✅ **実装済み（2026-06-14）** — 各提案アイテムのおすすめ商品（楽天/Yahoo!）から「価格に反映する商品」をラジオで選ぶと、AI概算価格ではなく選択した実商品の価格で合計金額・予算差・予算内訳チャートを再計算する。選択UIは**アイテムごとのラジオ**（`0=AI概算 / 1..=各商品`）で、既存のHTML商品カードは表示用に残置。選択状態は `st.session_state`（キー `sel_{idx}`）が rerun をまたいで保持し、描画のたびに `selected_products`（item_name → `ShoppingItem`）へ集約。合計は **`_eff_price()`（選択済み=実価格・未選択=AI概算）のハイブリッド**で算出し、アイテム見出し（「選択中」バッジ付き）・メトリクス・予算バー・チャートすべてに反映。レポート3形式（HTML/PDF/Excel）も `selections` 引数で実価格合計・選択商品の明示（HTML/PDFは「選択商品」行、Excelは✅＋太字）に対応。選択は新規生成時に全リセット、個別再生成時は該当 `sel_{idx}` のみリセット。i18n キーは5言語追加、`tests/smoke_test.py` に検証 [4b]/[7] を追加済み。
2. **間取り図アップロードへの対応** — 現在の部屋写真アップロード（Img2Img参照）は実写を想定しているが、間取り図（平面図）をアップロードしても機能するようにする。間取り図からは部屋の形状・窓/ドア位置を読み取り、配置アドバイスと完成予想図に反映するのが理想。実装時の想定: アップロード画像が実写か間取り図かを vision（gpt-4o mini）で分類し、画像生成プロンプトと参照画像の扱いを切り替える。`OpenAIImageGenerator._edit_from_reference`（未使用のImg2Img雛形）が関連コード。
3. **部屋タイプの選択 + 複数部屋の一括提案（コスト上限つき）** — 生成対象の部屋タイプ（リビング・キッチン・寝室・書斎・ダイニング等）を条件として選べるようにし、さらに1度の処理で複数の部屋をまとめて提案できるようにする。**設計上の制約: 運用コスト（OpenAI API の従量課金）が計1万円に収まること**（※「1万円」の範囲が総額か月額か、社内イベント期間中か等は実装前にユーザーへ要確認）。実装時の想定: 部屋タイプは i18n に言語別選択肢を追加してAIプロンプトへ反映。複数部屋は部屋ごとに生成パイプラインを回し、商品検索・画像生成は既存の並列化を流用。コストは画像生成が支配的（gpt-image-1 medium 1536×1024 ≒ 数円〜十数円/枚。テキスト生成は1円未満/回）のため、1回=部屋数×約10円が目安（3部屋で約20〜40円 → 1万円で250〜500回程度）。コスト制御の選択肢: 同時生成部屋数の上限、画像は1部屋1枚固定、quality を low/medium に制限、画像生成をオフにできるトグル、累計生成回数の記録。
4. **きちんとしたモバイル端末対応** ✅ **CSS対応を実装（2026-06-14。実機確認は残）** — `@media (max-width: 640px)`（app.py 内の `st.markdown` スタイル）を包括的に拡充した: 横余白の最適化（`.block-container` の左右パディング）、セクション見出し（`.sec-hd`）の縮小、**おすすめ商品カードの横スクロール化**（`.sim-row` を `flex-wrap: nowrap` + `overflow-x: auto` + scroll-snap、`.sim-card` 固定150px）、**タップ領域44px**（`stButton`/`stDownloadButton`/`button[kind]` に `min-height:44px`、新旧 testid 両対応）、メトリクス縦積み時の余白調整、サイドバーのブランドカード（`.sb-brand`）のコンパクト化。`st.columns` はスマホ幅で自動的に縦積みになるため「縦積み化」の作業は不要で、縦積み時の見た目最適化が主作業。サイドバーは `set_page_config` 既定の `initial_sidebar_state="auto"` によりスマホで自動折りたたみ済み。予算チャート（`st.bar_chart`）はコンテナ幅追従で responsive。**残タスク: 実機（iPhone/Android）での目視確認。** なお保存レポート（HTML）はビューポート＋flex-wrap で別途モバイル閲覧可。
5. **部屋のイメージカラーの選択** ✅ **実装済み（2026-06-14）** — 間取り・予算・テイストに加えて「部屋のイメージカラー（ベースカラー/アクセントカラー）」を条件として選べるようにした。i18n に言語別カラープリセット（`COLOR_BASE_OPTIONS`/`COLOR_ACCENT_OPTIONS`、各8色×5言語）を追加し、テイストと同型の「プリセット＋自由入力」セレクトボックス2つ（ベース／アクセント）をメインフォームの予算行の下に設置。各色の既定は「おまかせ（指定なし）」で、未指定なら色条件ブロックを出さず従来出力を維持する（非侵襲）。`prompt._color_block()` が `build_coordinate_prompt`・`build_single_item_prompt`（個別再生成）に色条件を注入し、提案アイテムの色味・room_image_prompt に反映。**search_keyword には色を含めない**（色で絞るとヒット率が下がるためプロンプトで明示的に禁止）。レポート3形式（HTML/PDF/Excel）の提案条件にもベース/アクセント行を追加。`gen_ctx` に `base_color`/`accent_color` を保持し、結果表示・レポートは生成時条件基準を維持。
