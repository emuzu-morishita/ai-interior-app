...
5. **部屋のイメージカラーの選択** — 間取り・予算・テイストに加えて「部屋のイメージカラー（ベースカラー/アクセントカラー）」を条件として選べるようにする。実装時の想定: i18n に言語別カラープリセット（ホワイト系・ベージュ系・グレージュ・ネイビー・グリーン等）を追加し（自由入力 or st.color_picker 併用も検討）、`build_coordinate_prompt` に色条件を追加して提案アイテムの色味・room_image_prompt に反映する。search_keyword に色を含めるかは要検討（色で絞ると検索ヒット率が下がるため、含めない方が無難）。
