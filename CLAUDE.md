...
4. **きちんとしたモバイル端末対応** — 現状は最小限のメディアクエリ（ヒーロー縮小・商品カード2列化）のみ。残タスクの想定: スマホ幅で st.columns が縦積みになった際の入力フォーム・メトリクス3連・詳細設定の見た目最適化、タップしやすいボタンサイズ（44px目安）、おすすめ商品カードの横スクロール化の検討、予算チャートのモバイル表示、サイドバー（スマホでは初期折りたたみ）内の導線見直し、実機（iPhone/Android）での確認。Streamlit はレイアウト制御に限界があるため CSS メディアクエリ中心。社長・社員はスマホで閲覧する可能性が高く、イベント（6/30）前の優先度は高め。
